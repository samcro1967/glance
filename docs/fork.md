...
- **Expanded native themes** — Expands Glance native theming from a small set of global colors into a semantic visual theme system covering colors, typography, page backgrounds, headers, navigation, widgets, widget headers, cards, groups and tabs, controls, footers, and elevated surfaces. The top-level `theme` remains the configured default appearance, while individual pages can provide partial theme overrides that inherit all unspecified values from the selected base theme. The theme picker provides the built-in Glance Dark and Glance Light themes together with explicitly named user themes. Theme values use validated, bounded configuration options, including local `/assets/...` page background images, while `custom-css-file` remains available as the final styling layer for advanced customization beyond the native theme model. Existing theme configurations remain compatible.
