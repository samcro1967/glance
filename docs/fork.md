...
- **Lazy image failure recovery** — Handles failed lazy-loaded images as a terminal presentation state and completes their normal transition cleanup, preventing failed image requests from leaving affected images permanently hidden or repeatedly treated as pending lazy loads.
