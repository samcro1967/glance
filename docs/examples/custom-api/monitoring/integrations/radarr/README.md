...
This integration retrieves monitoring information from Radarr and converts it to the shared [Custom API Monitoring](../../README.md) JSON contract.
...
![Movies monitoring example](../../../../../images/examples/custom-api/monitoring/radarr.png)
...
- A Radarr API accessible from the system running the producer

## Configuration

The producer is configured with environment variables:

- `RADARR_URL` — Radarr base URL. Defaults to `http://localhost:7878`.
...
The producer reports the total number of Radarr movies, monitored movies, downloaded movies, wanted movies, queue size, queued data size, and Radarr version.

A successful Radarr collection produces an overall `ok` state even when monitored movies are wanted. Wanted content is monitoring information rather than an application-health failure, so the Wanted metric and individual wanted movies are highlighted as warnings without degrading the overall widget state.

The expandable Wanted Movies section lists up to 20 monitored movies that do not currently have a file.
...
- type: custom-api
  title: Movies
  url: https://example.com/radarr_metrics.json
  cache: 5m
  headers:
...
[Glance README](../../../../../../README.md) · [Configuration](../../../../../configuration.md) · [Widgets](../../../../../widgets.md) · [Examples](../../../../../examples.md) · [Custom API Monitoring](../../README.md) · [Back to top](#movies)
