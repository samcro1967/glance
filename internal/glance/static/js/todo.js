...
export default async function(element) {
    ...
    const state = createPersonalState("todo", id);
    const initialItems = await state.load([], Array.isArray);

    element.swapWith(Todo(state, initialItems));
}
...
function Todo(state, initialItems) {
    ...
    const saveItems = () => {
        ...
        state.save(items.children.map(item => item.component.serialize()));
    };
    ...
    items = elem()
        .classes("todo-items")
        .append(
            ...initialItems.map(data => newItem(data))
        );
